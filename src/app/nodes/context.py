...
- user_id: (string) same as provided
- topics: (list of strings) distinct topics recently studied
- recent_findings: (list of strings) short summaries of recent findings
- outstanding_questions: (list of strings) unresolved research questions
"""
)
...
def summarize_context(user_id: str, topic: str) -> ContextSummary:
    """Summarize recent user activity into a structured context summary."""
    # Load last 10 briefs
    history: list[FinalBrief] = load_user_history(user_id, limit=10)

    # Convert history to readable JSON string
    history_text = "\n".join(
    ...
    summary: ContextSummary = parser.parse(response.content)
    return summary
